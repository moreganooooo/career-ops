...
### LaTeX templates (removed)

LaTeX template support has been removed from the default pipeline in favor of the Playwright HTML→PDF workflow. The HTML template `cv-template.html` and `generate-pdf.mjs` remain the supported path for ATS-optimized CV generation. If you relied on the `.tex` workflow, you can recover removed files from your VCS history.
