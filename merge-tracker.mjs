...
function validateStatus(status) {
  ...
  const aliases = {
    ...
    'skip': 'SKIP', 'monitor': 'SKIP', 'duplicate': 'Discarded', 'dup': 'Discarded', 'repost': 'Discarded',
    'geo blocker': 'SKIP',
  };
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const col4LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|duplicate|dup|repost|conditional|hold|monitor)/i.test(col4);
    const col5LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|duplicate|dup|repost|conditional|hold|monitor)/i.test(col5);
